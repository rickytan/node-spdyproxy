...
var SPDYProxy = function(options) {
  var self = this;
  this.whitelist = options.whitelist ? require(path.join(process.cwd(), options.whitelist)) : [];
  this.whitecity = ['hangzhou', 'changsha', 'beijing'];
  ...
  function handleRequest(req, res) {
    ...
    console.log("%s:%s".yellow + " - %s - " + "stream ID: " + "%s".yellow + " - priority: " + "%s".yellow,
      socket.connection ? socket.connection.socket && socket.connection.socket.remoteAddress : socket.socket.remoteAddress,
      socket.connection ? socket.connection.socket && socket.connection.socket.remotePort : socket.socket.remotePort,
    ...
    */
    ...
    var city = geoip.lookup(clientip);
    ...
    if ((!self.whitelist.some(function(str) {
        ...
        typeof self._authHandler == 'object') { // an AuthHandler is defined
